...
class FatJetSel(Module):##inherit from Module
    def __init__(self,jetid=1,minpt=200.0, maxeta=2.4, max_tau21=0.4, fatjetColl="CleanFatJet"):
    ...
    def beginFile(self, inputFile, outputFile, inputTree, wrappedOutputTree): ##wrappedOutputTree = Events
        ...
        for br in oBrList:
            ...
            if re.match('\A'+self.fatjetColl+'_', bname):
                if btype not in self.CollBr: self.CollBr[btype] = [] ## add branch type if not added yet
                self.CollBr[btype].append(bname)
                self.out.branch(bname, btype, lenVar='n'+self.fatjetColl)    ## initialize output branch ##remove original ones?
    ...
    def analyze(self, event):
        ...
        fatjet_coll = Collection(event, self.fatjetColl )
        nFatJet     = fatjet_coll._len
        if 'Clean' in self.fatjetColl : ori_fatjet_coll = Collection(event, self.fatjetColl.replace('Clean',''))
        order = []
        for iFatJet in range(nFatJet):
          ...
          if 'Clean' in self.fatjetColl :
              jetId = ori_fatjet_coll[fatjet_coll[iJet]['jetIdx']]['jetId'] ##jetid of original(nanoAOD)jet
          else:
              jetId = fatjet_coll[iJet]['jetId']
